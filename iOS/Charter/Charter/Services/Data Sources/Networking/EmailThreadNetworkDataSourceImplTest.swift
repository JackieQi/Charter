...
class EmailThreadNetworkDataSourceImplTest: XCTestCase {
    ...
    func testGetThreads() {
        let getsThreads = expectation(description: "getsThreads")
        let correctRequest = expectation(description: "correctRequest")
        ...
        mockSession.assertionBlockForRequest = { request in
            ...
            let expected = "http://charter.ws:8080/charter/emails?sort_by=-date&page=1&pagesize=25&filter=%7BinReplyTo:null%7D".characters.sorted()
            let actual = request.url!.absoluteString.characters.sorted()
